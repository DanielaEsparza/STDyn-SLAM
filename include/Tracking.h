...
public:
    ...
    cv::Mat GrabImageStereo(const cv::Mat &imRectLeft,const cv::Mat &imRectRight, const cv::Mat &imD, const double &timestamp);
...
public:
    ...
    // Current Frame
    Frame mCurrentFrame;
    cv::Mat mImGray, mImGrayRight;
    cv::Mat mImS;
    cv::Mat mImS_C;
    cv::Mat mImRGB;// adding for color point map  by zoe
    cv::Mat mImDepth; // adding mImDepth member to realize pointcloud view

    //To visualize keypoints on the image gray
    std::vector<cv::KeyPoint> mvKeys, mvKeysRight, mvKeysPreGraph, mvKeysDespGraph;
