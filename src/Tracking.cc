...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const cv::Mat &imD, const double &timestamp)
{
    mImGray = imRectLeft;
    ...
    if(mImGray.channels()==3)
    {
        if(mbRGB)
        {
            ...
            cvtColor(mImGrayRight,mImGrayRight,CV_RGB2GRAY);
        }
        else
        {
            cvtColor(mImGray,mImGray,CV_BGR2GRAY);
            cvtColor(imGrayRight,imGrayRight,CV_BGR2GRAY);
            cvtColor(mImGrayRight,mImGrayRight,CV_BGR2GRAY);
        }
    }
    else if(mImGray.channels()==4)
    {
        if(mbRGB)
        {
            ...
            cvtColor(mImGrayRight,mImGrayRight,CV_RGBA2GRAY);
        }
        else
        {
            cvtColor(mImGray,mImGray,CV_BGRA2GRAY);
            cvtColor(imGrayRight,imGrayRight,CV_BGRA2GRAY);
            cvtColor(mImGrayRight,mImGrayRight,CV_BGRA2GRAY);
        }
    }
    ...
    // If images are loaded from path and the poincloud is not visible, uncomment the next line
    // if it does not work, please modify the mDepthMapFactor in your file.txt
    //mImDepth*=0.1;

	

    mCurrentFrame = Frame(mImGray,imGrayRight,mImDepth,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mThDepth);
	...
	mCurrentFrame.CalculEverythingStereo(mImRGB,mImGray,imGrayRight,mImDepth,mpSegment->mImgSegmentLatest);
...
void Tracking::StereoInitialization()
{
    if(mCurrentFrame.N>500)
    {
        ...
        cout << "New map stereo created with " << mpMap->MapPointsInMap() << " points" << endl;
...
void Tracking::CreateNewKeyFrame()
{
    ...
    mpPointCloudMapping->insertKeyFrame( pKF,this->mImRGB,this->mImS ,this->mImRGB, this->mImDepth );
