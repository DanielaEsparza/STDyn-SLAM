...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imRight, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight, ORBVocabulary* voc, const float &thDepth)
     ...
     mpReferenceKF(static_cast<KeyFrame*>(NULL))
{
    ...
    thread threadLeft(&Frame::ExtractORBKeyPoints,this,0,imGray);
...
void Frame::CalculEverythingStereo(cv::Mat &imRGB,const cv::Mat &imGray, const cv::Mat &imRight, const cv::Mat &imDepth,
	const cv::Mat &imS)
{ 
    ...
    for ( int m=0; m<imS.rows; m+=1 )
    {
        for ( int n=0; n<imS.cols; n+=1 )
        {
            ...
            if(labelnum == PEOPLE_LABLE || labelnum == 1 || labelnum == 2 || labelnum == 3 || labelnum ==6 || labelnum ==7 || labelnum == 8
            || labelnum == 12 || labelnum == 13 || labelnum == 14 || labelnum ==19)
            {
                flagprocess=1;
                break;
            }
    ...
    if(!T_M.empty() && flagprocess )
    {
        ...
        cout << "check time stereo =" << tc*1000 <<  endl;
    }

	
    thread threadLeft(&Frame::ExtractORBDesp,this,0,imGray);
    thread threadRight(&Frame::ExtractORBDesp,this,1,imRight);
    threadLeft.join();
    threadRight.join();
...
{ 
	...
	//cout << "Frame 330, mvKeys: " << mvKeys.size() << "  mvKeysTemp: " << mvKeysTemp.size() << endl;
    if(!T_M.empty() && flagprocess )
    {
        std::chrono::steady_clock::time_point tc1 = std::chrono::steady_clock::now();
        flag_mov = mpORBextractorLeft->CheckMovingKeyPoints(imGray,imS,mvKeysTemp,T_M);
...
void Frame::ExtractORBDesp(int flag,const cv::Mat &imgray)
{
    if(flag==0){
        ...
        (*mpORBextractorRight).ProcessDesp(imgray,cv::Mat(),mvKeysTempRight,mvKeysRight,mDescriptorsRight);
}
...
void Frame::ComputeStereoFromRGBD(const cv::Mat &imDepth)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        const float d = imDepth.at<float>(v,u)*1000;
