			   ...
			   mbDeactivateLocalizationMode(false)
{
    ...
    float depthMaxPC = fsSettings["PointCloudMapping.DepthMaxPC"];

    depthMaxPC = (depthMaxPC==0) ? 5.0 : depthMaxPC;
    ...
    mpPointCloudMapping = boost::make_shared<PointCloudMapping>( resolution, depthMaxPC );
...
cv::Mat System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const cv::Mat &depthmap, const double &timestamp)
{
    if(mSensor!=STEREO)
    {
    ...
    return mpTracker->GrabImageStereo(imLeft,imRight,depthmap,timestamp);
}
