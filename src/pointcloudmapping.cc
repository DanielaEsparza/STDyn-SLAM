...
bool flag_pc_finish=true;
double depthMaxPC;

PointCloudMapping::PointCloudMapping(double resolution_, double depthMaxPc_)
{
    this->resolution = resolution_;
    depthMaxPC=depthMaxPc_;
...
void PointCloudMapping::insertKeyFrame(KeyFrame* kf, cv::Mat& semantic_color,cv::Mat& semantic,cv::Mat& color, 
	cv::Mat& depth)
{
...
bool PointCloudMapping::flagFinishPC()
{
	return flag_pc_finish;
}
            ...
            if (d < 0.01 || d > depthMaxPC || d!=d)
		        ...
		        for (int i=-20;i <= 20; i+=3)
		        {
                    for (int j=-20;j <= 20; j+=3)
                    {
                        ...
                        if((int)semantic.ptr<uchar>(tempx)[tempy] == PEOPLE_LABLE || (int)semantic.ptr<uchar>(tempx)[tempy] == 1 || 
						(int)semantic.ptr<uchar>(tempx)[tempy] == 2 || (int)semantic.ptr<uchar>(tempx)[tempy] == 3 || (int)semantic.ptr<uchar>(tempx)[tempy] ==6 || 
						(int)semantic.ptr<uchar>(tempx)[tempy] ==7 || (int)semantic.ptr<uchar>(tempx)[tempy] == 8 || (int)semantic.ptr<uchar>(tempx)[tempy] == 12 || 
						(int)semantic.ptr<uchar>(tempx)[tempy] == 13 || (int)semantic.ptr<uchar>(tempx)[tempy] == 14 || (int)semantic.ptr<uchar>(tempx)[tempy] ==19)  
                        {
...
void PointCloudMapping::viewer()
{
    ...
    ros::NodeHandle nh;
    pclPoint_pub = nh.advertise<sensor_msgs::PointCloud2>("/ORB_SLAM2_PointMap_SegNetM/Point_Clouds",100000);
    ...
    while(1)
    {
		...
		cout << "PointCloudmapping KfMap " << depthImgs.size() << endl;
