...
{
    ros::init(argc, argv, "STDyn_KITTI");
    ros::start();
    ...
    while(ros::ok() && ni<nImages)
    {
        ...
        imDepth = cv::Mat::zeros(imRight.rows, imRight.cols, CV_64F);
        
        if (foldertime.is_open())
        {
			getline(foldertime, tp);
			ros_time= boost::lexical_cast<double>(tp);
        }
        
        double tframe = ros_time;
        if(imLeft.empty())
        {
    ...
    SLAM.SaveTrajectoryKITTI(trajectory_KITTI);
